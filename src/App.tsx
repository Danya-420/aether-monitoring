...
declare global {
    interface Window {
        aetherAPI: {
            ...
            onToggleIncognito: (callback: (val: boolean) => void) => () => void;
            onSystemResume: (callback: () => void) => () => void;
            enterSleepMode: () => void;
            enableLoopbackAudio: () => Promise<void>;
            disableLoopbackAudio: () => Promise<void>;
            sendAudioActivity: (isActive: boolean) => void;
        }
    }
}
...
export default function App() {
    ...
    const refreshData = async () => {
        try {
            ...
            if (data.settings) {
                if (data.settings.isIncognito !== undefined) {
                    setIsIncognito(data.settings.isIncognito);
                }
                if (data.settings.autoLaunch !== undefined) {
                    setAutoLaunchEnabled(data.settings.autoLaunch);
                }
            }
        } catch (error) {
    ...
    useEffect(() => {
        ...
        // Audio Detection State (Scoped to useEffect)
        let audioContext: AudioContext;
        let analyser: AnalyserNode;
        let source: MediaStreamAudioSourceNode;
        let pcmData: Float32Array;
        let audioIntervalId: any;
        ...
        const cleanupAudio = () => {
            ...
            if (audioContext && audioContext.state !== 'closed') {
                audioContext.close().catch(e => console.error('[Audio] Close failed:', e));
            }
            if (stream) {
        ...
        const setupAudioLoopback = async () => {
            try {
                cleanupAudio();
                console.log('[Audio] Requesting loopback stream...');
                await window.aetherAPI.enableLoopbackAudio();
                ...
                // Remove video tracks to avoid bugs
                stream.getVideoTracks().forEach(track => {
                    track.stop();
                    stream?.removeTrack(track);
                });

                await window.aetherAPI.disableLoopbackAudio();
                console.log('[Audio] Successfully acquired loopback stream');

                audioContext = new AudioContext();

                analyser = audioContext.createAnalyser();
                analyser.fftSize = 256;
                pcmData = new Float32Array(analyser.frequencyBinCount);

                source = audioContext.createMediaStreamSource(stream);
                source.connect(analyser);

                // Re-initialize if the stream is ever lost 
                stream.getAudioTracks().forEach(track => {
                    track.onended = () => {
                        console.warn('[Audio] Track ended, re-initializing loopback...');
                        setupAudioLoopback();
                    };
                });
                ...
                const checkAudioLevel = async () => {
                    ...
                    if (audioContext.state === 'suspended' && Date.now() - lastResumeAttempt > 5000) {
                        lastResumeAttempt = Date.now();
                        await audioContext.resume().catch(e => console.error('[Audio] Resume failed:', e));
                    }

                    if (audioContext.state === 'running' && pcmData) {
                        analyser.getFloatTimeDomainData(pcmData as any);
                        let sumSquares = 0.0;
                        ...
                        const hasInstantSignal = rms > 0.005;

                        if (hasInstantSignal) {
                            lastActiveTime = currentTime;
                        }

                        // Audio is considered active if signal was detected within the grace period
                        const isActive = currentTime - lastActiveTime < GRACE_PERIOD;

                        if (isActive !== lastReportedState) {
                            console.log(`[Audio] Monitoring state: ${isActive ? 'ACTIVE' : 'INACTIVE'} (Signal: ${hasInstantSignal}, RMS: ${rms.toFixed(4)})`);
                            setIsAudioActive(isActive);
                            window.aetherAPI.sendAudioActivity(isActive);
                            lastReportedState = isActive;
                        }
                    }
                };

                audioIntervalId = setInterval(checkAudioLevel, 100);

            } catch (error) {
                console.error('[Audio] Failed to initialize loopback:', error);
                await window.aetherAPI.disableLoopbackAudio().catch(() => { });
            }
        };

        setupAudioLoopback();

        const removeIncognitoToggle = window.aetherAPI.onToggleIncognito((val: boolean) => {
            setIsIncognito(val);
        });

        const removeSystemResume = window.aetherAPI.onSystemResume(() => {
            console.log('[Audio] System resumed, re-initializing loopback to ensure stability...');
            setupAudioLoopback();
        });
        ...
        return () => {
            ...
            if (removeIncognitoToggle) removeIncognitoToggle();
            if (removeSystemResume) removeSystemResume();
                                ...
                                {isAudioActive && (
                                    <>
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="font-mono text-[10px] tracking-widest text-orange-400">AUDIO_ACTIVE</span>
                                        </div>
                                    </>
                                )}
                                <div className="w-px h-6 bg-stone-700"></div>
                                                    ...
                                                    <div className="bg-stone-50 border-2 border-stone-200 p-5">
                                                        ...
                                                        {(!wpmStats || (wpmStats as any).weeklyAvg === 0) ? (
                                                            ...
                                                            <>
                                                                ...
                                                                {/* Trend Indicator */}
                                                                <div className={`flex items-center gap-2 mt-4 pt-4 border-t-2 border-stone-200 ${(wpmStats as any).trend > 0 ? 'text-orange-600' :
